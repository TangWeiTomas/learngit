...
int wireless_ap_disable(bool st)
{
	...
	{
		snprintf(cmd,128,"uci set wireless.ap.encryption='none'\0");
		system(cmd);
		snprintf(cmd,128,"uci del wireless.ap.key");
		system(cmd);
		snprintf(cmd,128,"uci set wireless.ap.hidden='0'\0");
		system(cmd);
	}
