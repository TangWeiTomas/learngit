...
static void zblist_Process_Handler_cb(void *args)
{
	...
	switch(zblist->deviceid)
	{
		case ZB_DEV_METER_ENERGY_DEVICE:
			meter_SendSerialCmd(epInfo->nwkAddr,epInfo->endpoint,afAddr16Bit,zblist->cmdlen,zblist->cmd[zblist->cmdlen - 1]);
		break;
		...
		case ZCL_HA_DEVICEID_FXZB_DOORLOCK_YAOTAI:
			doorLockYT_setOnOffReq(epInfo,NULL);
		break;
...
//			break;
		...
		case ZCL_HA_DEVICEID_WINDOW_COVERING_DEVICE:
			zbSoc_NewCurtainDevCtrlCmdReq(zblist->cmd[zblist->cmdlen - 1],epInfo->nwkAddr,epInfo->endpoint,afAddr16Bit);
		break;
