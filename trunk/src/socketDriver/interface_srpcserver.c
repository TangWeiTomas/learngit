...
static uint8_t SRPC_ConfigSsidPasswd(hostCmd * cmd)
{
...
#if (SUPPORT_VERSION_CC)

	log_debug("SUPPORT_VERSION_CC\n");
	...
	if(iwpriv_isFound())
	{
		...
		wireless_sta_config(ssid, NULL, pwd, NULL);
...
static uint8_t SRPC_permitJoin(hostCmd * cmd)
{
	...
	if (openTimes)
		zbSoc_Permit_Join_Req(60);
	else 
		zbSoc_Permit_Join_Req(0x00);
...
int8_t SRPC_FactorySettingReq(epInfo_t * epInfo)
{
	...
	if (epInfo != NULL) //是0X010A门锁或主控
	{
		...
		{
			//zbSoc_RevertOneDevFactorySettingCmd(epInfo->nwkAddr,epInfo->IEEEAddr);
			zbSoc_RevertOneDevFactorySettingCmd(epInfo->nwkAddr, epInfo->endpoint);
		}
		else 
		{
			//zbSoc_RevertOneDevFactorySettingCmd(srcepInfo.nwkAddr,srcepInfo.IEEEAddr);
...
static uint8_t SRPC_RevertOneFactorySettingReq(hostCmd * cmd)
{
	...
	if (epInfo != NULL)
	{
		...
		{
			//Revert One Device Factory Sertting
			//zbSoc_RevertOneDevFactorySettingCmd(epInfo->nwkAddr,epInfo->ieee);
...
uint8_t SRPC_MtUtilSetChannelsReq(hostCmd * cmd)
{
	...
	static tu_evtimer_t mtUtilSetChannelsTimers =
	{
...
static uint8_t SRPC_QueryDevOnlineStateReq(hostCmd * cmd)
{
	...
	//	HeartPacketSend();
	//ZbSocHeartbeat_HeartPacketSend();
	ZbSocHeartbeat_OnLineDeviceResp();
...
static int8_t SRPC_addPeriodTimerSwitchCtrlReq(hostCmd * cmd)
{
	...
	nodeMembersRecord_t member[MAX_SUPPORTED_NODE_MEMBERS] =
	{
		0
	};
	...
	if (timerTaskInfo.timeTaskType != EVENT_TYPE_SCENEID_ACTION &&
		 timerTaskInfo.timeTaskType != EVENT_TYPE_NODE_ACTION)
	{
		log_err("add error\n");
...
static uint8_t SRPC_AddDevSceneReq(hostCmd * cmd)
{
	...
	//场景节点的描述，最大节点数量为100个
	sceneMembersRecord_t member[MAX_SUPPORTED_NODE_MEMBERS] =
	{
		0
	};
...
static uint8_t SRPC_WinCurtainCtrlReq(hostCmd * cmd)
{
	...
	if (epInfo != NULL)
	{
		...
		//新协议接口窗帘控制
		if(epInfo->deviceID == ZCL_HA_DEVICEID_WINDOW_COVERING_DEVICE || epInfo->deviceID == ZCL_HA_DEVICEID_WINDOW_COVERING_DEVICE_FXKJ)
		{
			uint8_t cmds = 0;
			if(ctrlCmd != 0x03)
			{
				if(ctrlCmd == 0x04)
					cmds = 0x00;
				else if(ctrlCmd == 0x05)
					cmds = 0x01;
				else
					cmds = 0x02;

				zbSoc_NewCurtainDevCtrlCmdReq(cmds, epInfo->nwkAddr, endPoint, afAddr16Bit);
			}
			//查询状态
			else
			{
				if(epInfo->deviceID == ZCL_HA_DEVICEID_WINDOW_COVERING_DEVICE)
					zbSoc_NewCurtainGetPercentCmdReq(epInfo->nwkAddr, endPoint, afAddr16Bit);
				else 
					zbSoc_NewCurtainGetStatusCmdReq(epInfo->nwkAddr, endPoint, afAddr16Bit);
			}
			
		}
		else if (epInfo->deviceID == ZB_DEV_WIN_CURTAIN)
		{
...
static uint8_t SRPC_WinCurtainPercentageReq(hostCmd * cmd)
{
	...
	if (epInfo != NULL)
	{
		...
		if(epInfo->deviceID == ZCL_HA_DEVICEID_WINDOW_COVERING_DEVICE || epInfo->deviceID == ZCL_HA_DEVICEID_WINDOW_COVERING_DEVICE_FXKJ)
		{
			zbSoc_NewCurtainDevPercentageCmdReq(percentage, epInfo->nwkAddr, endPoint, afAddr16Bit);
		}
		else if (epInfo->deviceID == ZB_DEV_WIN_CURTAIN)
		{
...
static uint8_t SRPC_ComAlarmCtrlReq(hostCmd * cmd)
{
	...
	if (epInfo != NULL)
	{
		...
		{
			SRPC_ComAlarmCtrlCfm(ieeeAddr, endPoint, YY_STATUS_FAIL);
			return FAILED;
		}
...
static uint8_t SRPC_ComAlarmGetPowerReq(hostCmd * cmd)
{
	...
	if (epinfo != NULL)
	{
		...
		if ((epinfo->deviceID == ZB_DEV_DOOR_SENSOR) || (epinfo->deviceID == ZB_DEV_INFRARED_BODY_SENSOR) ||
			 (epinfo->deviceID == ZCL_HA_DEVICEID_IAS_ZONE))
		{
	...
	if (epInfo != NULL)
	{
		...
		switch (epInfo->deviceID)
		{
			...
			case ZCL_HA_DEVICEID_FXZB_DOORLOCK_YAOTAI:
				doorLockYT_setOnOffReq(epInfo, switchCmd);
				break;
		}
	}
	else 
...
static uint8_t SRPC_LevelDoorLockUsrReq(hostCmd * cmd)
{
	...
	if (epInfo != NULL)
	{
		...
		switch (epInfo->deviceID)
		{
			...
			case ZCL_HA_DEVICEID_FXZB_DOORLOCK_YAOTAI:
				//doorLockYT_usrMngReq(epInfo,usrType,optType,usrid,passwd);
				break;
		}
...
#define 	CENTRAL_AIR_REAL_DATA_SIZE 9
	...
	if (epInfo != NULL)
	{
		if (epInfo->onlineflag == true)
		{
			uint8_t value[CENTRAL_AIR_REAL_DATA_SIZE] =
			{
				0
			};
...
void SRPC_ActionEventAddReq(hostCmd * cmd)
{
	...
	Event_t member[MAX_SUPPORTED_NODE_MEMBERS] =
	{
		0
	};
...
{
	uint8_t rec[MAX_SUPPORTED_RECORD_SIZE] =
	{
		0
	};
	uint8_t devNum		= 0;
...
static uint8_t SRPC_PermissionRequest_Resp(hostCmd * cmd)
{
	...
	uint8_t PermiType	= 0;
	uint8_t PermiData[PERMI_BUF_SIZE] =
	{
		0
	};
	struct tm tp;
...
void SRPC_MeterReqCfm(uint8_t * ieeeAddr, uint8_t endpoint, uint8_t status)
{
	hostCmd cmd;
	cmd.idx 			= 0;
	makeMsgHeader(&cmd, CMD_MSG_DIR_CFM);
	...
	//D0 D1 Opcode
	cmdSet16bitVal(&cmd, WIFI_ZIGBEE_SET_METER_CFM);
...
void SRPC_WaterMeterReqCfm(uint8_t * ieeeAddr, uint8_t endpoint, uint16_t opcode, uint8_t status)
{
	hostCmd cmd;
	cmd.idx 			= 0;
	makeMsgHeader(&cmd, CMD_MSG_DIR_CFM);
...
static int8_t SPRC_SetMeterReqCmd(hostCmd * cmd)
{
	uint8_t ieeeAddr[8];
	uint8_t endPoint;
	epInfo_t * epInfo	= NULL;

	log_debug("SPRC_SetMeterReqCmd++\n");
	...
	{
		log_err("vdevListGetDeviceByIeeeEp device no fonud \n");
		SRPC_MeterReqCfm(ieeeAddr, endPoint, YY_STATUS_NODE_NO_EXIST);
		return YY_STATUS_NODE_NO_EXIST;
	}

	if (epInfo->onlineflag == true)
	{
		SRPC_MeterReqCfm(ieeeAddr, endPoint, YY_STATUS_SUCCESS);
	}
	else 
	{
		SRPC_MeterReqCfm(ieeeAddr, endPoint, YY_STATUS_OUTLINE);
	}
...
void SRPC_DoorLockDevice(uint16_t opcode, hostCmd * cmd)
{
	...
	{
		//门锁控制
		case DOORLOCK_CTRL_REQ:
			SRPC_DoorLockCtrlReqCB(opcode, cmd);
			break;

		case DOORLOCK_GET_POWER_REQ:
			SRPC_DoorLockPowerReqCB(opcode, cmd);
			break;

		case DOORLOCK_GET_INFO_REQ:
			SRPC_DoorLockDeviceInfoReqCB(opcode, cmd);
			break;

		case DOORLOCK_PWD_MNG_REQ:
			SRPC_DoorSetPINCodeReqCB(opcode, cmd);
			break;

		case DOORLOCK_CARD_MNG_REQ:
			SRPC_DoorSetRFIDCodeReqCB(opcode, cmd);
			break;

		case DOORLOCK_FIGNER_MNG_REQ:
			SRPC_DoorLockSetFingerCodeReqCB(opcode, cmd);
			break;

		case DOORLOCK_GET_OPEN_TYPE_REQ:
			SRPC_DoorLockGetDoorOpenPatternReqCB(opcode, cmd);
			break;

		case DOORLOCK_SET_OPEN_TYPE_REQ:
			SRPC_DoorLockSetDoorOpenPatternReqCB(opcode, cmd);
			break;

		case DOORLOCK_GET_POLLRATE_REQ:
			SRPC_DoorLockGetShortPollRateReqCB(opcode, cmd);
			break;

		case DOORLOCK_SET_POLLRATE_REQ:
			SRPC_DoorLockSetShortPollRateReqCB(opcode, cmd);
			break;
...
*				电表控制器(44H)				*
************************************************/
void SRPC_MeterEnergyDevice(uint16_t opcode, hostCmd * cmd)
{
	uint8_t ieeeAddr[8];
	uint8_t endPoint;
	epInfo_t * epInfo	= NULL;
	...
	{
		log_err("vdevListGetDeviceByIeeeEp device no fonud \n");
		SRPC_WaterMeterReqCfm(ieeeAddr, endPoint, opcode, YY_STATUS_NODE_NO_EXIST);
		return;
	}

	if (epInfo->onlineflag == true)
	{
		SRPC_WaterMeterReqCfm(ieeeAddr, endPoint, opcode, YY_STATUS_SUCCESS);
	}
	else 
	{
		SRPC_WaterMeterReqCfm(ieeeAddr, endPoint, opcode, YY_STATUS_OUTLINE);
	}

	meter_serverProcess(opcode, epInfo, cmd);
}
...
*				水表(44H)				*
************************************************/
void SRPC_WaterMeterDevice(uint16_t opcode, hostCmd * cmd)
{
	uint8_t ieeeAddr[8];
	uint8_t endPoint;
	epInfo_t * epInfo	= NULL;
	...
	waterMeter_serverProcess(opcode, epInfo, cmd);
}
...
void SRPC_ProcessIncoming(hostCmd * cmd)
{
	...
	switch (opcode & 0xFF00)
	{
		...
		case WIFI_METER_DEVICE_ID:
			SRPC_MeterEnergyDevice(opcode, cmd); //METER_ENERGY
			break;

		case WIFI_WATER_METER_DEVICE_ID:
			SRPC_WaterMeterDevice(opcode, cmd);
			break;
