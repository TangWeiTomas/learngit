...
uint8_t main_parse_cmd(int argc,char**argv)
{
	...
	while((cmd = getopt_long(argc,argv,"vVhHdD",cmd_optons,NULL))!=-1)
	{
		switch(cmd)
		{
			...
			case 'V':
				{
					printf("Version: %s  SVN:%d  (%s , %s)\n",SUB_VERSION,VERSION_NUMBER,__DATE__,__TIME__);
					return 1;
				}
				break;
			case 'h':
...
int main(int argc,char**argv)
{
	...
	if (signal(SIGCHLD, sig_chld) == SIG_ERR) 
	{
        	log_err("signal error to SIGCHLD");
    	}
...
#if 0
	if(sim800lEvt_evtInit("/dev/ttyS1"))
	{
		log_debug("sim800l_evtInit failed\n");
		goto done;
	}
	...
	if(watchdogInit())
	{
		log_err("watchdogInit error\n");
		goto done;
	}
#if 0
//#if SUPPORT_YINJIA_HEATBEAT
	if(zbSoc_Heartbeat_YinJia_Report_start()!=true)
//	if(zbSoc_Heartbeat_DeviceList_Report_start()!=true)
	{
		log_debug("zbSoc_Heartbeat_DeviceList_Report_start failed\n");
		goto done;
